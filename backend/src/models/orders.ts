...
export default class Order {
	...
	async deleteOneHeader(id: OheaderType['id']): Promise<Record<string, unknown>> {
		const connection = await client.connect()
		//delete rows first
	...
	async userOrders(user_id: OheaderType['user_id']): Promise<Record<string, unknown>[]> {
		const connection = await client.connect()
	...
	async details(id: OheaderType['id']): Promise<Record<string, unknown>[]> {
		const connection = await client.connect()

...
describe('Testing orders', () => {
    ...
    it('other deletes delete all entries', async () => {
        ...
        const c2 = await customers.deleteOneCustomer(cus[1].id)
        const c3 = await customers.deleteOneCustomer(cus[2].id)
        const cusf = await customers.readAllCustomers()

        const usr = await users.readAllUsers()
        const u1 = await users.deleteOneUser(usr[0].id)
        const u2 = await users.deleteOneUser(usr[1].id)
        const u3 = await users.deleteOneUser(usr[2].id)
        ...
        const p2 = await products.deleteOneProduct(pro[1].id)
        const p3 = await products.deleteOneProduct(pro[2].id)
